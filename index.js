...
export const debounce = (func, wait, options) => DebouncePromise(func, wait, options);
...
export class DebounceCache {
  ...
  getDebouncedFunction = (func, wait, options, args) => {
    ...
    if (!this.debounceCache[cacheKey]) {
      let debouncedFunc = debounce(func, wait, otherOptions);
      if (onlyResolvesLast) {
        debouncedFunc = onlyResolvesLast(debouncedFunc);
      }
      this.debounceCache[cacheKey] = debouncedFunc
    }
    return this.debounceCache[cacheKey];
  }
}
...
function AwesomeDebouncePromise(func, wait, options) {
  ...
  return function AwesomeDebouncePromiseWrapper(...args) {
    const debouncedFn = debounceCache.getDebouncedFunction(func, wait, finalOptions, args);
